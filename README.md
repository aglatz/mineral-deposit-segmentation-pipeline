...
To demonstrate how segmentation method works a MATLAB script 'segment_us_phantom.m' is provided in ${GITROOTDIR}/libBRIC/mineral-deposit-segmentation/test/ that segments the lesions of our lesion phantom. The structural T1- and T2*-weighted volumes of the lesion phantom can be found in data, where the script will place its output as well.

Steps for running the script:
1) cd /to/some/new/directory
2) download and extract NIFTI[15] and LIBRA[16] toolboxes for MATLAB
3) git clone git@github.com:aglatz/mineral-deposit-segmentation-pipeline.git
4) cd libBRIC/mineral-deposit-segmentation/test/
5) open 'segment_us_phantom.m' adjust the two 'addpath()' lines, which include the external libraries
6) run 'segment_us_phantom.m'
7) cd data
8) fslview -m single GRE_restore.nii.gz T1W_restore.nii.gz NonTis_mask.nii.gz NormTis_mask.nii.gz Oli_usnew_mask.nii.gz

The last three volumes were generated by the software. The volumes are:
...
- Oli_usnew_mask is the mask that selects the lesions of the lesion phantom.

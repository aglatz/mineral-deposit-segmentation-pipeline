...
- cd libBRIC/mineral-deposit-segmentation/test/
- open 'segment_us_phantom.m' adjust the two 'addpath()' lines, which include the external libraries
- run 'segment_us_phantom.m'
- cd data
- fslview -m single GRE_restore.nii.gz T1W_restore.nii.gz NonTis_mask.nii.gz NormTis_mask.nii.gz Oli_usnew_mask.nii.gz

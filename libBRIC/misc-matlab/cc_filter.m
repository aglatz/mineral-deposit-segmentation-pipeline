function [S_hypos, S_hypos_hypo, S_hypos_hyper, App] = ...
	cc_filter(S_gre, S_t1w, N_gre, S_roi, SM_hypos, ...
              S_ntis, I_thr, CNR_thr, intvar_thr)
...
fprintf('Before: %d; CNR_thr=%0.2f intvar_thr=%0.2f\n', ...
        sum(SM_hypos(:)), CNR_thr, intvar_thr);
...
for lab_idx = 1:N_lab
    ...
    % Determine appearance of CC
    vol = sum(SM_cc(:));
    [phypo, SM_cc_hypo] = get_phypo(SM_cc, S_t1w, I_thr(M, 4));
    [phyper, SM_cc_hyper] = get_phyper(SM_cc, S_t1w, I_thr(M, 5));
    CNR = abs(median(S_gre(SM_cc))-median(S_gre(SM_ntis)))/N_gre;
    ...
    % Make decision ...
    if CNR >= CNR_thr && intvar >= intvar_thr
        App(lab_idx, end) = 1; % included
        % all
        S_hypos = S_hypos + cast(SM_cc, class(S_hypos)) .* Loc(lab_idx);
        % hypo hypo
        S_hypos_hypo = S_hypos_hypo + cast(SM_cc_hypo, class(S_hypos_hypo)) .* Loc(lab_idx);
        % hypo hyper
        S_hypos_hyper = S_hypos_hyper + cast(SM_cc_hyper, class(S_hypos_hyper)) .* Loc(lab_idx);
        fprintf('*\n');
    else
        App(lab_idx, end) = 0;
        fprintf('\n');

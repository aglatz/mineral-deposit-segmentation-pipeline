...
X_new = F_old(1):F_new(1):(size(S_old, 1)*F_old(1));
X_new = F_old(1)+(max(X_old)-max(X_new))/2:F_new(1):(size(S_old, 1)*F_old(1));
%
Y_new = F_old(2):F_new(2):(size(S_old, 2)*F_old(2));
Y_new = F_old(2)+(max(Y_old)-max(Y_new))/2:F_new(2):(size(S_old, 2)*F_old(2));
%
Z_new = F_old(3):F_new(3):(size(S_old, 3)*F_old(3));
Z_new = F_old(3)+(max(Z_old)-max(Z_new))/2:F_new(3):(size(S_old, 3)*F_old(3));
